...
1. **Route: `GET /admin` — admin-only, returns full page or HTMX panel**
   ...
   - The route mount registers `/admin` + `/admin/health` + `/admin/users` + `/admin/reference-data` + `/admin/trash` + `/admin/system` — the sub-routes return the panel fragment only (HTMX or direct — direct GET of `/admin/health` returns the fragment with status 200; no full-page re-render for sub-paths). Rationale: `hx-push-url` pushes `/admin?tab=<name>`, but the HTMX `hx-get` targets `/admin/<name>` — cleaner than query-string HTMX with complex `hx-include`.
