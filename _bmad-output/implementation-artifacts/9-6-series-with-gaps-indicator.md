...
2. **AC2 — Anonymous CAN use `/?filter=gaps` (ASYMMETRIC vs unshelved/overdue — series browsing is anonymous-permitted per FR65 + FR95).** This is the load-bearing deviation from the 9-4/9-5 anonymous-no-leak pattern. Series listings ARE anonymous-readable (FR95 — `series_list_page` at `src/routes/series.rs:82` requires no auth). Therefore:
   ...
   - **Implementation:** the role gate at `home.rs:145-149` becomes per-variant. After parsing, gate Unshelved + Overdue on `role >= Librarian` but allow Gaps for any role. The `active_indicator_filter` Vec/Option used to drive `build_indicator_tags` carries the role-gated value (so Anonymous never gets a tag emitted at count=0), while a separate `gaps_filter_active: bool` (computed from the raw parser result, role-blind) drives the section swap.
