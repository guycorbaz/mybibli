...
status: done
created: 2026-04-29
last_updated: 2026-04-29 (code-review pass 1 batch-applied — 21 patches landed, status review → done; awaiting CI validation of new wizard-E2E lane and AC14 happy-path integration test)
...
- 2026-04-29 — code-review pass 1 (BMAD bmad-code-review, 3 adversarial layers): findings written to **Review Findings** section below.
- 2026-04-29 — code-review pass 1 batch-apply: all 21 patches applied (15 P1–P15 + 6 from D1–D6 resolved option (a)). Validated locally: `cargo check --all-targets`, `cargo clippy --all-targets -- -D warnings` clean; 28 unit tests across `services::setup`, `services::admin_system`, `middleware::setup_gate`, `routes::setup` pass. New integration test `tests/setup_wizard.rs::full_happy_path_step_1_through_login` and the wizard E2E spec require a DB / CI run to validate end-to-end. P6 marked N/A: `tests/docker-compose.rust-test.yml` only defines the DB service, the rust-test runner runs host-side and controls gate state via `state_with_pool(active: true)`; no env var injection point applies. Notable additions beyond the patch list: `tests/e2e/docker-compose.wizard.yml` override + `e2e-wizard` CI job in `_gates.yml`.

### Review Findings

> Code review run on 2026-04-29 via the BMAD `bmad-code-review` skill — 3 parallel adversarial layers (Blind Hunter / Edge Case Hunter / Acceptance Auditor) over the full branch diff (~3631 lines, 37 files) against `main`. Critical functional bug found: the wizard is unreachable past Step 1 in production — see P1 below. P1 + P2 must be fixed before this story can ship.

#### Decision-needed (resolved 2026-04-29 — all chose option (a); see corresponding patches P16–P21 below)

- [x] [Review][Decision] **D1 — Resolved (a):** wire `update_existing_admin` into Step 1 idempotent re-submit path (after P1 lands). See **P16**.
- [x] [Review][Decision] **D2 — Resolved (a):** add per-row Skip checkboxes to Step 2 per AC5 (3 checkboxes + matching `Step2Form` fields + handler logic). See **P17**.
- [x] [Review][Decision] **D3 — Resolved (a):** wire Step 1 idempotent re-render so Step 2 Previous reaches Step 1 in update-mode (paired with D1/P16). See **P18**.
- [x] [Review][Decision] **D4 — Resolved (a):** wrap the 3 provider-key writes in `save_provider_keys` in a single transaction. See **P19**.
- [x] [Review][Decision] **D5 — Resolved (a):** flash a one-shot banner via cookie/session for the `admin_already_created` loser-branch. See **P20**.
- [x] [Review][Decision] **D6 — Resolved (a):** fail-closed in `SetupGateState::initialize` — panic on DB error at boot so the operator sees the failure. See **P21**.

#### Patches (to fix or check off)

- [x] [Review][Patch] **P1 — CRITICAL: wizard is unreachable past Step 1 — `resolve_step` short-circuits on `admin_count > 0`** [`src/services/setup.rs:77-99`] — After Step 1 commits an admin row, `resolve_step` returns `None` (rule 2: "admin already exists from a pre-Epic-8 install → inactive"). `routes/setup.rs::resolve_active_step` propagates that as `Err(NotFound)`, so GET `/setup` returns 404. Step 2/3/4 are unreachable in production. The unit test `resolve_step_admin_exists_returns_none` asserts the bug rather than catching it; the integration test `step_1_creates_admin_and_resolver_advances` only checks the 303 from the POST, not the next GET; the E2E spec that would have caught this is gated behind `MYBIBLI_SETUP_E2E=1` (P2). Fix: in `resolve_step`, only short-circuit on `setup_completed_at.is_some()` — drop the `admin_count > 0` early-return so `resolve_active_step` can narrow via `resolve_step_with_admin`. Update the unit test accordingly. Add an integration test covering the GET-after-Step-1 transition.
- [x] [Review][Patch] **P2 — CRITICAL: no CI E2E coverage of the multi-step wizard journey (Foundation Rule #7)** [`tests/e2e/specs/journeys/setup-wizard.spec.ts:32-38`] — `test.skip(MYBIBLI_SETUP_E2E !== "1", ...)` with no CI workflow setting that env var. The wizard's smoke test is effectively dead in CI; P1 above slipped through because of this. Fix: add a Playwright project / docker-compose lane that spawns the test stack without `MYBIBLI_SKIP_SETUP=1`, runs only this spec, and is wired into the `e2e` job in `.github/workflows/_gates.yml`. Alternative: spawn the wizard-mode container from within the spec's `webServer` config so the gate becomes self-contained.
- [x] [Review][Patch] **P3 — `KEYED_PROVIDERS` declared in routes/setup.rs instead of metadata/mod.rs (AC5 + Files-to-create)** [`src/routes/setup.rs:54`, `src/metadata/mod.rs`] — Spec AC5 line 116 + Dev Notes line 469 both name `metadata::KEYED_PROVIDERS` as the single source of truth; the doc-comment in `routes/setup.rs:52-53` even refers to a phantom `metadata::KEYED_PROVIDERS`. Fix: move the const to `src/metadata/mod.rs`, re-export from `routes/setup.rs` if convenient, update `routes/admin_system.rs` to import from the same source.
- [x] [Review][Patch] **P4 — Rule-of-three extraction not done in routes/admin_system.rs (Task 3 + AR9)** [`src/routes/admin_system.rs:180-220`] — `services/admin_system.rs` was created (good) but `routes/admin_system.rs` still has its own private `save_setting`, `reload_settings_cache`, `validate_default_language`. The duplication this story was supposed to retire is intact. Fix: remove the in-route helpers and route everything through `crate::services::admin_system`.
- [x] [Review][Patch] **P5 — Step 1 admin session cookie has 7-day Max-Age — should be no-max-age (session cookie)** [`src/routes/setup.rs:664-670`] — Login flow at `routes/auth.rs:217-220` mints authenticated cookies with NO `max_age` (session cookie); the wizard sets `.max_age(time::Duration::days(7))` which converts the cookie to a persistent one. CLAUDE.md "Session" section documents the project pattern. Fix: drop the `.max_age()` call.
- [x] [Review][Patch] **P6 — `tests/docker-compose.rust-test.yml` not updated with `MYBIBLI_SKIP_SETUP=1` (Task 8 + Files-to-modify)** [`tests/docker-compose.rust-test.yml`] — Tests rely on `force_set_active` to compensate. Fix: add `MYBIBLI_SKIP_SETUP: "1"` to the mybibli service in this compose file (matches the change made to `tests/e2e/docker-compose.test.yml` and `docker-compose.dev.yml`).
- [x] [Review][Patch] **P7 — `force_set_active` is `pub fn` without `#[cfg(test)]`** [`src/middleware/setup_gate.rs:161-164`] — Production callers can flip the gate cache silently. Fix: gate behind `#[cfg(any(test, feature = "test-helpers"))]` or move into a `pub mod test_helpers` annotated `#[cfg(test)]`.
- [x] [Review][Patch] **P8 — `complete_setup` INSERT path can race on duplicate-key with concurrent invocations** [`src/services/setup.rs:413-424`] — Concurrent Step-4 submits both reach the missing-row branch and both attempt INSERT; one fails with 23000. Fix: use `INSERT INTO settings (setting_key, setting_value) VALUES (?, ?) ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value), version = version + 1`.
- [x] [Review][Patch] **P9 — `save_preferences` silently no-ops if a settings row is missing** [`src/services/setup.rs:367-393`] — Each `if let Some(...)` skips the write entirely if the row is missing; the `setup_step_3_done` sentinel write is the most damaging case (resolver loops back to Step 3 forever). Fix: log a warning and INSERT the missing row, OR migrate to a transactional `INSERT ... ON DUPLICATE KEY UPDATE` per P8.
- [x] [Review][Patch] **P10 — `fetch_predicate_inputs` and `setup_gate::fetch_active` parse `setup_completed_at` differently** [`src/services/setup.rs:163-169` vs `src/middleware/setup_gate.rs:125-128`] — Resolver requires successful RFC3339 parse; gate treats any non-empty string as completed. A garbage timestamp value (manual DB intervention) disables the gate but keeps the resolver active → inconsistent state with possible redirect-loop. Fix: align — adopt the strict RFC3339 parser in both, or extract to a shared helper.
- [x] [Review][Patch] **P11 — Username/password length checks count bytes, not chars** [`src/routes/setup.rs:605-610`, `src/services/setup.rs:209`] — `username.len() > USERNAME_MAX_LEN` and `password.len() < PASSWORD_MIN_LEN` count UTF-8 bytes; HTML `maxlength` counts chars. Multi-byte unicode usernames/passwords get rejected by the server even when the HTML accepts them. Fix: use `.chars().count()` for parity.
- [x] [Review][Patch] **P12 — `render_page` and `render_page_with_status` are duplicated** [`src/routes/setup.rs:373-430`] — Same body except status code + dead `let _ = step;` binding in both. Fix: collapse into a single function taking `StatusCode` (default 200), delete the dead binding.
- [x] [Review][Patch] **P13 — `complete_submit` comment claims HTMX-aware redirect but emits unconditional 303** [`src/routes/setup.rs:825-828`] — AC7 says HTMX `HX-Redirect`, non-HTMX 303; implementation is unconditional `Redirect::to("/catalog")`. The form is a plain submit so practical impact is nil. Fix: either add the `if is_htmx_request → HX-Redirect` branch, or rewrite the comment to acknowledge the unconditional 303 is intentional.
- [x] [Review][Patch] **P14 — No integration test for AC14 happy path (step-2 → step-3 → step-4 → /setup is 404 → POST /login works)** [`tests/setup_wizard.rs`] — Current 6 tests cover gate redirect, whitelist, GET render, Step 1 success, Step 1 password rejection, post-completion 404 (faked via direct SQL + `force_set_active`). The actual multi-step flow is never exercised — directly downstream of P1. Fix: add a `#[sqlx::test]` walking step-1 → step-2 → step-3 → /setup/complete → re-GET /setup is 404 → POST /login with the new admin works.
- [x] [Review][Patch] **P15 — `services::auth::generate_session_token` doc-comment claims URL-safe base64 but uses STANDARD** [`src/services/auth.rs:25-31`] — Project convention is STANDARD (consistent with `middleware/auth.rs:112`); only the doc-comment is wrong. Fix: amend the doc-comment to "STANDARD base64 (matches `routes/auth.rs::generate_session_token`)".
- [x] [Review][Patch] **P16 — Wire `update_existing_admin` into Step 1 idempotent re-submit path (resolves D1)** [`src/routes/setup.rs::step_1_submit`, `src/services/setup.rs::update_existing_admin`] — After P1 (resolver no longer short-circuits on `admin_count > 0`), Step 1 re-renders should pre-fill `username_value` with the existing admin row's username + show the `setup.step_1_admin_exists_hint` + flip the submit label to `setup.step_1_update_button`. On submit with empty password ⇒ keep current; with non-empty password ⇒ rehash. Re-use `UserModel::update` via the existing `update_existing_admin` helper. Add unit + integration test for the update path.
- [x] [Review][Patch] **P17 — Add per-row Skip checkbox to Step 2 (resolves D2)** [`templates/fragments/setup_step_providers.html`, `src/routes/setup.rs::Step2Form`] — Add three `<input type="checkbox" name="skip_<provider>">` controls (one per provider row) per AC5. Update `Step2Form` to deserialize `skip_google_books / skip_omdb / skip_tmdb: bool` (default false). Wire the AC5 contract: skipping leaves the row at empty string ONLY if previously empty; an existing key submitted with Skip checked is NOT cleared. Add i18n keys `setup.step_2_skip_<provider>_label`.
- [x] [Review][Patch] **P18 — Step 2 Previous returns to Step 1 in idempotent-update mode (resolves D3)** [`src/routes/setup.rs::step_2_submit`] — Once P16 lands, Step 2 `_back=true` 303s to `/setup` and the resolver narrows via `resolve_step_with_admin` BUT the wizard needs to honor "user clicked Previous" by ignoring the resolver's narrowing for one cycle. Implementation options: (a) pass `?previous=1` to GET /setup, OR (b) flash-cookie `setup_previous_target=admin` cleared after one render. Update Task 5 going-backward semantics docs accordingly. Drop the "UX-acceptable" comment at `src/routes/setup.rs:702-704`.
- [x] [Review][Patch] **P19 — Wrap `save_provider_keys` in a single transaction (resolves D4)** [`src/services/setup.rs::save_provider_keys`] — Begin a transaction, call `save_setting` against `&mut *tx` for each of the 3 providers, commit at the end. If any write errors, rollback and return the error — partial writes never persist. Update `save_setting` signature if needed to accept `&mut sqlx::MySqlConnection` (or extract the transactional-aware variant). Add a unit/integration test covering rollback.
- [x] [Review][Patch] **P20 — Flash banner for `admin_already_created` race (resolves D5)** [`src/routes/setup.rs::step_1_submit` Conflict branch] — On the losing-browser branch (lines 649-654), set a one-shot session-attribute or `flash_message=` cookie carrying the i18n key `setup.errors.admin_already_created_by_other_browser`, 303 to `/setup`. The next GET `/setup` reads + clears the flash and renders the message inline above the panel as a localized banner (CSP-clean, server-rendered). Add an integration test covering the race outcome.
- [x] [Review][Patch] **P21 — Fail-closed in `SetupGateState::initialize` (resolves D6)** [`src/middleware/setup_gate.rs:72-95`] — Replace the `Err(e) => { warn; false }` branch with `Err(e) => panic!("setup-gate: cannot determine wizard state from DB: {e}")` (or `std::process::exit(1)` after a fatal log). The boot must abort so the operator notices and fixes the DB before the install proceeds. Update the doc-comment + Dev Notes accordingly.

#### Deferred (file as GitHub Issues per Foundation Rule #11)

- [x] [Review][Defer] **W1 — `strip_masked` rejects legit values starting with bullets** [`src/routes/setup.rs:710-722`] — Practical impact essentially zero (4 bullet chars in any realistic API key alphabet). File as `type:code-review-finding`.
- [x] [Review][Defer] **W2 — Cookies missing `Secure` flag** [`src/routes/setup.rs:664-670`, `src/routes/auth.rs:217-220`] — Codebase-wide pattern, not introduced by this story. File as a separate hardening issue (`type:code-review-finding`).
- [x] [Review][Defer] **W3 — DRY: `setup_completed_at` parsed in two places** [`src/services/setup.rs`, `src/middleware/setup_gate.rs`] — P10 fixes the correctness aspect; the DRY refactor is a cleanup follow-up.
- [x] [Review][Defer] **W4 — AC13 concurrent-first-launch race has no E2E test** — Hard to simulate cleanly in Playwright; AC13 is asserted via the single-flight unit test today. File as `type:code-review-finding`.
- [x] [Review][Defer] **W5 — `render_step_done` defaults non-fr/en language values to FR display** [`src/routes/setup.rs:319-322`] — Implicit fallback hides DB corruption; preferences validation prevents non-fr/en values from being written. Practical impact nil.

#### Dismissed (false positives / out-of-scope, not persisted)

- CSRF middleware does NOT validate form-body tokens — **FALSE.** `src/middleware/csrf.rs:180-209` parses `_csrf_token` from the urlencoded body when no `X-CSRF-Token` header is present; tests `form_field_token_passes_when_header_absent` and `missing_token_returns_403` cover both branches.
- `bare.html` `csrf_token` requirement breaks login template — **FALSE.** `LoginTemplate` already declares `csrf_token: String` (`src/routes/auth.rs:25`); no regression.
- Single-flight admin guard not actually atomic — **DISMISS.** UNIQUE constraint on `users.username` is the actual single-flight enforcement; the COUNT+INSERT pattern is documented in spec as defense-in-depth, not the primary guard.
- Step 2/3/complete reachable as anonymous — **DISMISS.** By design per AC13 (the wizard runs anon for browsers other than the one that ran Step 1); each handler self-gates via `resolve_active_step`.
- `MYBIBLI_SKIP_SETUP` whitespace handling — **DISMISS.** Strict accept-set is by design (story 8-7 R3-N6 lesson, baked into AC9).
- Cache TOCTOU race — **DISMISS.** Per spec Dev Notes #9 "Cache invalidation race accepted".
- Locale fallback for unsupported `Accept-Language` — **DISMISS.** `locale_resolve_middleware` handles fallback before reaching the wizard handler.
- Session token UNIQUE collision — **DISMISS.** Astronomically unlikely with 32 random bytes.
