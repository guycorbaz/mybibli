...
2. **Docker Hub publishing — semver tags ONLY** — Docker Hub pushes happen exclusively from `release.yml` on `v*.*.*` tag pushes. **No** `docker-publish` job runs on `main` pushes (per Guy's "version stricte" decision: images are produced only at Epic boundaries when a semver is cut, never mid-Epic). `ci.yml` ends after the 3 gates.
3. **Tag release flow** — `.github/workflows/release.yml` triggered by `v*` tags that (a) verifies `Cargo.toml` version matches the tag, (b) runs the same 3 gates, (c) builds and pushes `gcorbaz/mybibli:<semver>` + `gcorbaz/mybibli:latest`.
...
6. **No Docker Hub publish on `main` push (semver-only policy):** Given a push to `main`, when CI runs, then `ci.yml` runs ONLY the 3 gate jobs and exits — no `docker-publish` job exists. Docker Hub images are produced exclusively by `release.yml` on `v*.*.*` tag pushes (see AC #7/#8). Rationale (Guy 2026-04-13 "version stricte"): images ship per-Epic semver release, never per mid-Epic story merge — keeps Docker Hub free of throwaway snapshots.
...
10. **README badge:** The `README.md` top section gets a CI status badge: `![CI](https://github.com/guycorbaz/mybibli/actions/workflows/ci.yml/badge.svg?branch=main)` (the `?branch=main` filter pins the badge to main and avoids flicker from feature-branch runs). Optional: a Docker Hub pulls badge (skip if it bloats the README — Guy's call).
...
- [x] **Task 2 — REMOVED — no `docker-publish` job in `ci.yml`** (AC: #5, #6)
  - Per Guy 2026-04-13 "version stricte" decision: Docker Hub publishing happens ONLY in `release.yml` on semver tag pushes. `ci.yml` ends after the 3 gates. Original Task 2 (4th `docker-publish` job tagging `main-<sha7>`) was implemented then removed (commit pending). All Docker push concerns now consolidated in Task 3 (`release.yml`).
...
- [x] **Task 4 — Configure GitHub repo** (AC: #1, #3, #5)
  - [x] 4.1 In GitHub Settings → Secrets and variables → Actions, add `DOCKERHUB_USERNAME` (value: `gcorbaz` — Docker Hub namespace, NOT GitHub username `guycorbaz`) and `DOCKERHUB_TOKEN` (value: a Docker Hub PAT with push scope on `gcorbaz/mybibli`, created at https://hub.docker.com/settings/security). Rotate-and-document procedure goes into `docs/ci-cd.md` (Task 6). **DONE 2026-04-13.**
  - [x] 4.2 In GitHub Settings → Branches → Add rule for `main`: require status checks to pass — **EXACTLY these three** (display names from `_gates.yml` `name:` fields, NOT job IDs): `gates / Rust tests + clippy + sqlx-prepare`, `gates / DB integration tests`, `gates / Playwright E2E`. **DONE 2026-04-14.** **🚨 DO NOT add `docker-publish` to required checks** — it is skipped on PRs (`if: github.ref == 'refs/heads/main' && github.event_name == 'push'`). A skipped check never reports a status, so marking it required would make every PR permanently unmergeable. Same rule applies to release-only jobs in `release.yml`. Require branches to be up to date. Do NOT require reviews (solo-maintainer mode — Guy approves their own PRs). Allow force-push: OFF. Allow deletion: OFF. Include administrators: ON (so Guy can't bypass accidentally).
...
2. Click "New Access Token", scope: "Read, Write, Delete" on `gcorbaz/mybibli` Docker Hub repo only (not account-wide — least privilege). Note: Docker Hub namespace is `gcorbaz` (different from GitHub username `guycorbaz`).
3. Copy the token once (it's not shown again) into GitHub repo Settings → Secrets → Actions as `DOCKERHUB_TOKEN`. Add `DOCKERHUB_USERNAME=gcorbaz` as a companion secret.
...
2. `.github/workflows/ci.yml` — replaces the old single-job workflow. Calls `_gates.yml` ONCE via a single `gates` parent job (initial implementation called it three times, which caused 409 artifact-name conflicts on the `e2e` upload — fixed in commit `ebd7649`). No `docker-publish` job: per Guy 2026-04-13 "version stricte" decision, Docker Hub publishing is consolidated in `release.yml` only. `ci.yml` ends after the 3 gates.
...
- **Task 4.2** — Configure branch protection on `main` per `docs/ci-cd.md#branch-protection-for-main`. The three required checks must be selected as `gates / Rust tests + clippy + sqlx-prepare`, `gates / DB integration tests`, `gates / Playwright E2E` (single `gates` parent job calls the reusable `_gates.yml`; GitHub reports each reusable-workflow inner job as `<parent-job-id> / <called-job-display-name>`). Do NOT add `docker-publish` or release-only jobs to required checks — they skip on PRs and would lock merges.
...
- `.github/workflows/ci.yml` (replaced — previously single-job; `docker-publish` removed in semver-only revision)
- `.github/workflows/release.yml` (new)
- `docs/ci-cd.md` (new — `docs/` directory also new)
- `README.md` (modified — CI badge + docs link)
- `.gitignore` (modified — removed bare `package-lock.json` pattern that was excluding `tests/e2e/package-lock.json` from git and breaking `setup-node` cache)
- `tests/e2e/package-lock.json` (new — committed for CI npm-cache reproducibility; previously gitignored)
...
- 2026-04-13 — Commit `8e248f1`: initial implementation pushed (3 gates + docker-publish + release.yml).
- 2026-04-13 — Commit `ebd7649`: fix 409 artifact-name conflict by collapsing the 3 calls of `_gates.yml` in `ci.yml`/`release.yml` into a single `gates` parent job. Updated docs/ci-cd.md branch-protection check names accordingly (`gates / Rust tests + clippy + sqlx-prepare` etc.).
- 2026-04-13 — Commit `3d2a6fb`: commit `tests/e2e/package-lock.json` and remove bare `package-lock.json` from `.gitignore` (was breaking `setup-node`'s `cache-dependency-path`).
- 2026-04-13 — Commit `d4ce4b4`: fix Docker Hub namespace `guycorbaz/mybibli` → `gcorbaz/mybibli` across `ci.yml`, `release.yml`, `docs/ci-cd.md`. (GitHub username is `guycorbaz`; Docker Hub namespace is `gcorbaz` — distinct accounts.) First fully-green CI run + Docker push confirmed.
- 2026-04-14 — Story validation pass: corrected stale references (Docker namespace, `_gates.yml` call count, secret value, README badge, AC #2 check naming). **AC #6 inverted** to "no `docker-publish` on main push" per Guy's "version stricte" decision (Docker Hub gets only semver tags from `release.yml`); `docker-publish` job removed from `ci.yml`. Task 2 marked REMOVED.

<!-- Open Questions moved to "Decisions to Resolve BEFORE Implementation" at the top of the story. -->

### Review Findings

_Code review 2026-04-14 (Blind Hunter + Edge Case Hunter + Acceptance Auditor on commits `8e248f1..a11d5ba`)._

- [x] [Review][Patch] `latest` tag overwritten by older-semver hotfix [release.yml:64-66] — Resolved 2026-04-14: switch to `docker/metadata-action` with `type=semver,pattern={{version}}` for the semver tag + `type=raw,value=latest` guarded so it only updates when the pushed tag is the highest semver on the repo (e.g., fetch all tags and compare, or skip `latest` if any existing tag sorts higher). Implementation choice during fix.
- [x] [Review][Decision] Force-pushed tag silently mutates published Docker image — **ACCEPTED 2026-04-14** as a solo-maintainer trade-off. No precheck added. Documented as known behavior; revisit if project gains contributors.
- [x] [Review][Patch] `.gitignore` global `package-lock.json` removed — scope regression [.gitignore:line removed] — the bare pattern was removed to un-ignore `tests/e2e/package-lock.json`. Any future npm subdir now commits its lockfile accidentally. Fix: replace bare pattern with explicit `!tests/e2e/package-lock.json` negation or scope to `/package-lock.json` at repo root.
- [x] [Review][Patch] `publish` job missing `actions: write` permission for GHA Docker build cache [release.yml publish job] — workflow-level `permissions: contents: read` is inherited. `docker/build-push-action@v5` with `cache-to: type=gha,mode=max` requires `actions: write` to write the cache; silently degrades to no cache write. Add `permissions: { contents: read, actions: write }` on the `publish` job.
- [x] [Review][Patch] Artifact uploads skipped on job cancellation [_gates.yml:91,146,157] — all uploads use `if: failure()`. When a run is cancelled (concurrency or manual), no Playwright report is collected, defeating diagnosis of the cancelled flake. Change to `if: failure() || cancelled()`.
- [x] [Review][Patch] Artifact name collides across re-runs [_gates.yml upload steps] — name uses `${{ github.run_id }}` only; re-running the same run (Re-run jobs) reuses the id and collides. Fix: append `-${{ github.run_attempt }}`.
- [x] [Review][Patch] Playwright cache `restore-keys` can hydrate stale browser binaries [_gates.yml:117-119] — restore-keys fallback restores an old Chromium after a Playwright version bump, causing "Executable doesn't exist" errors. Fix: remove the `restore-keys:` fallback, or include the Playwright package version in the cache key (e.g., `hashFiles('tests/e2e/package-lock.json')` is already included; adding `${{ env.PLAYWRIGHT_VERSION }}` explicitly is cleaner).
- [x] [Review][Patch] `release.yml` missing `concurrency:` — two rapid tag pushes could race, two concurrent `publish` jobs could both push to `:latest`. Add `concurrency: { group: release-${{ github.ref }}, cancel-in-progress: false }` (do NOT cancel — let both complete; or cancel if you prefer).
- [x] [Review][Patch] Spec AC #2 narrative lists wrong composite check names — AC #2 says `gates / rust-tests`, `gates / db-integration`, `gates / e2e` (job IDs). `_gates.yml` defines `name:` display fields so the real composite names are `gates / Rust tests + clippy + sqlx-prepare`, `gates / DB integration tests`, `gates / Playwright E2E`. Task 4.2 and `docs/ci-cd.md` already use the correct display names (branch protection is therefore configured correctly). Fix: update AC #2 narrative + completion notes (line 207) + Change Log (line 231) to match reality, or remove the `name:` fields in `_gates.yml` to align with AC #2 wording.
- [x] [Review][Defer] Cargo.toml version parsing fragile vs workspace/BOM/CRLF/single-quotes [release.yml:30] — `grep '^version = "' | head -1` breaks if a `[workspace.package]` or dependency table with `version = "..."` appears before `[package]`, if file is UTF-8-BOM, if CRLF (leaves `\r` in var), or if single-quoted. Today Cargo.toml has none of these — deferred. Future fix: `cargo pkgid | cut -d'#' -f2 | cut -d'@' -f2` or `cargo metadata --format-version 1 --no-deps | jq -r '.packages[0].version'`.
- [x] [Review][Defer] `_gates.yml` caller does not pass `secrets: inherit` — footgun for future gate needing a secret [_gates.yml / ci.yml / release.yml call sites] — documented in `docs/ci-cd.md#known-gotchas`; no current failure. Address when a gate first needs a secret.
- [x] [Review][Defer] Task 3.5 tag-mismatch smoke test not executed — explicitly deferred in the story; acknowledged here for traceability.
