...
configuration séparé. Le panneau a six onglets :

\begin{itemize}
  \item \textbf{Health} — tableau de bord en lecture seule. Outre les
        versions, les compteurs d'entités et l'état des fournisseurs,
        il affiche le dernier numéro de volume (code V) et le dernier
        numéro d'étagère (code L) jamais imprimés, corbeille comprise,
        chacun avec le prochain numéro libre — à consulter avant
        d'imprimer une nouvelle planche d'étiquettes.
  \item \textbf{Users} — créer/désactiver des utilisateurs, changer
        de rôle.
  \item \textbf{Reference data} — les tables taxonomiques, et le
        vocabulaire des étiquettes de gestion
        (section~\ref{sec:management-labels}).
  \item \textbf{Trash} — voir, restaurer et supprimer définitivement
        les éléments soft-deleted.
  \item \textbf{System} — paramètres globaux.
  \item \textbf{API keys} — créer et révoquer les clés avec
        lesquelles l'API HTTP s'authentifie (chapitre~\ref{ch:api}).
\end{itemize}
...
\section{Étiquettes de dos : codes V et codes L}\label{sec:labels}

\begin{note}
\textbf{Deux choses s'appellent « étiquettes » dans mybibli.} Cette
section traite des \emph{étiquettes de dos} que l'on imprime et que
l'on colle sur un livre ou une étagère — les codes-barres qui rendent
le scan possible. L'autre sorte, apparue en 1.18.0, est
l'\emph{étiquette de gestion} que l'on pose sur une notice pour dire
quelque chose de son état (« À vérifier », « Reliure abîmée ») ; elles
sont décrites en section~\ref{sec:management-labels}. Rien ne s'imprime
pour elles, et elles ne sortent jamais de l'application.
\end{note}
...
\begin{description}
  ...
  \item[Étiquettes]\index{étiquettes} Votre vocabulaire de travail
        \string— « À vérifier », « À relire », « Reliure abîmée »,
        ce que votre pratique réclame. À la différence des quatre
        précédentes, une étiquette se pose \emph{à la fois} sur des
        titres et sur des volumes, et un même titre peut en porter
        plusieurs. Elles ne sont visibles que des bibliothécaires :
        un visiteur de passage n'en voit rien, nulle part.
\end{description}
...
via les bibliothèques}. Elle relance la recherche de métadonnées pour
\emph{tous} les titres catalogués disposant d'un code — pas seulement
ceux sans couverture — afin de remplir les champs alignés UNIMARC
introduits en 1.13.0 sur les titres catalogués avant la mise à jour.
Depuis la 1.14.0 elle interroge les deux bibliothèques nationales, la
BnF et la Library of Congress, ce qui étend son effet aux ouvrages
anglophones. Elle ne
