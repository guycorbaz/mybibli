...
19. **Release Doc Sync** — When releasing a new version (cutting a tag, bumping `Cargo.toml`, or shipping a release artifact), update the documentation in the same change-set to reflect the new version: at minimum the user manual under `docs/manual/{en,fr}/`, any version-referenced sections of `README.md`, and the `last_updated` header of `_bmad-output/implementation-artifacts/sprint-status.yaml` if it carries version semantics. The release is not done until the docs reflect it — a version bump that leaves the manual showing the previous version is a broken release.
