...
**Current stable: [`v1.7.7`](https://github.com/guycorbaz/mybibli/releases/tag/v1.7.7)** — the seventh patch on top of v1.7.0 ("Reach more users, debug more easily" — German + Italian UI translations and persistent log directory with rotation, shipped 2026-05-21). v1.7.7 ships a test-rigor bundle with one user-visible enhancement — the series detail page (`/series/:id`) now renders each position as a card with cover thumbnail + "Vol. N" prefix + title + author (#336) instead of a compact position cell — plus seven code-review-finding clean-ups closing silent E2E no-op patterns and locking concurrency / locale-resolution / single-active-filter contracts (#43, #62, #88, #89, #96, #119, #120). The originally-planned feature build for v1.1 through v1.6 is complete and v1.7 is currently the last themed minor on the roadmap; the project is in **GH-issue-driven polish mode** — no next themed minor is currently scoped. Patches ship independently for production bugs; new feature CRs are queued in the parking lot until a coherent theme emerges or a roadmap-slotted candidate (e.g., the v2.0 classification refactor) is greenlit.
