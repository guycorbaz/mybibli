...
**Current stable: [`v1.7.10`](https://github.com/guycorbaz/mybibli/releases/tag/v1.7.10)** — the tenth patch on top of v1.7.0 ("Reach more users, debug more easily" — German + Italian UI translations and persistent log directory with rotation, shipped 2026-05-21). v1.7.10 is a single-fix patch closing a silent-data-loss vulnerability: a manually uploaded cover (the v1.7.6 #335 safety net for titles that no provider has) was being silently overwritten by the async metadata-fetch chain because `update_cover_image_url` did an unconditional UPDATE that ignored `manually_edited_fields`. Fixed via a guard pattern identical to `do_update`'s — 22 lines of source plus 4 integration tests covering success / download-failure / regression / specificity paths (#347). The originally-planned feature build for v1.1 through v1.6 is complete and v1.7 is currently the last themed minor on the roadmap; the project is in **GH-issue-driven polish mode** — no next themed minor is currently scoped. Patches ship independently for production bugs; new feature CRs are queued in the parking lot until a coherent theme emerges or a roadmap-slotted candidate (e.g., the v2.0 classification refactor) is greenlit.
