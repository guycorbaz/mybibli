...
**Current stable: [`v1.7.9`](https://github.com/guycorbaz/mybibli/releases/tag/v1.7.9)** — the ninth patch on top of v1.7.0 ("Reach more users, debug more easily" — German + Italian UI translations and persistent log directory with rotation, shipped 2026-05-21). v1.7.9 is a cover-quality + admin-UX + 4-hardening bundle: Google Books cover URL now prefers the highest-resolution `imageLinks` variant (#333 piste 1, sharper covers + more titles attaching a cover after re-fetch); admin UI for the per-provider metadata-chain timeout + provider-health probe timeout hot-reloads from `/admin > System` without a restart (#334, pattern v1.7.1 #308); hardenings — anonymous-session purge persists `last_run_at` to survive crash-loops within 24 h (#46), observability trio for `register_loan` retry-log clarity + spawned-task panic capture + CLAUDE.md PoolTimedOut wording (#28), `templates_audit` robust comment-stripping + SVG masking + quote-aware regex (#24), and the 6-retro-old typing-race flake in `home-search.spec.ts:224` closed by switching the typing helper (#196). The originally-planned feature build for v1.1 through v1.6 is complete and v1.7 is currently the last themed minor on the roadmap; the project is in **GH-issue-driven polish mode** — no next themed minor is currently scoped. Patches ship independently for production bugs; new feature CRs are queued in the parking lot until a coherent theme emerges or a roadmap-slotted candidate (e.g., the v2.0 classification refactor) is greenlit.
...
- [#333](https://github.com/guycorbaz/mybibli/issues/333) — Cover-refetch bulk action insuffisante : 73/146 prod titles without a cover. **Piste 1 (Google Books `imageLinks` higher-res fallback) shipped in v1.7.9**; piste 2 (BDGest cover-only fetch by ISBN — the FR-publisher gap) still open. Re-validate prod miss rate after the v1.7.9 NAS upgrade before deciding whether piste 2 is needed.
