...
**Current stable: [`v1.18.0`](https://github.com/guycorbaz/mybibli/releases/tag/v1.18.0)** — a single-feature minor on v1.17.0 (shipped 2026-08-18): [#443](https://github.com/guycorbaz/mybibli/issues/443) **management labels**. An admin defines a vocabulary once — "À vérifier", "À relire", whatever the practice calls for — and a librarian applies it to titles *and* volumes from that one shared list; a dedicated `/labels` page lists the vocabulary with its counts, drills down to the members of each label in two sections (titles above volumes), and links through to their detail pages. Labels are internal: an anonymous visitor sees none of it, and the handlers do not even run the queries for them. **One additive migration** (`labels`, `title_labels`, `volume_labels`) — the first migration since v1.15.0, so a downgrade is a question worth asking again. The nav bar now switches to its menu below 1024px instead of 768px: the new entry brought an admin to eight links, which overflowed a tablet viewport by 86px. Shipped in three reviewed tranches (#473, #474, #475); see the v1.18.0 section below. The prior release [`v1.17.0`](https://github.com/guycorbaz/mybibli/releases/tag/v1.17.0) — a 2-issue metadata-clarity minor on v1.16.0 (shipped 2026-08-18): [#202](https://github.com/guycorbaz/mybibli/issues/202) a metadata lookup that finds nothing now says which of three things happened — every source searched and none holds the book, a source too busy to answer, or a source never asked because no API key is set — three situations that had shared one message and call for opposite reactions; [#206](https://github.com/guycorbaz/mybibli/issues/206) genre and Dewey are grouped into a Classification section that states the asymmetry between them (no fetch ever writes the genre; the Dewey code is proposed until you edit it yourself). **No migration.** Both issues came from the same production user, eight months apart, and both turned out to be about the interface telling the truth rather than the machinery being wrong; see the v1.17.0 section below. The prior release [`v1.16.0`](https://github.com/guycorbaz/mybibli/releases/tag/v1.16.0) — a 4-issue minor on v1.15.0 (shipped 2026-08-17): [#450](https://github.com/guycorbaz/mybibli/issues/450) K10plus, the German union catalogue, joins the chain as a zone completer and immediately becomes its leading contributor — 45 titles and 100 UNIMARC zones on the production catalog against the Library of Congress's 29 and 61 — gated by ISBN prefix (978-0/1/2/3) so only requests that can plausibly hit are paced; [#459](https://github.com/guycorbaz/mybibli/issues/459) `MYBIBLI_RESET_ADMIN`, a one-shot startup hatch that puts a locked-out administrator back in without a database console; [#457](https://github.com/guycorbaz/mybibli/issues/457) the proposed L-code now counts soft-deleted rows, so it is a code that is actually free; [#458](https://github.com/guycorbaz/mybibli/issues/458) four intermittently failing E2E specs deflaked — three distinct root causes — restoring the CI signal. **No migration.** Shipped through two release candidates: rc.1 was withdrawn before publication when its own gate caught a rate-limiter contention bug, and rc.2 ran a full production backfill whose logs settled the K10plus decision; see the v1.16.0 section below. The prior release [`v1.15.0`](https://github.com/guycorbaz/mybibli/releases/tag/v1.15.0) — a 4-issue minor on v1.14.1 (shipped 2026-08-13), bundling the August production-log review: [#202](https://github.com/guycorbaz/mybibli/issues/202) titles now record and display which provider resolved their metadata, so a thin record can be told apart from one no provider answered; [#424](https://github.com/guycorbaz/mybibli/issues/424) light-mode contrast raised above the WCAG AA floor on the home and locations pages, where the offending elements only rendered once data existed; [#419](https://github.com/guycorbaz/mybibli/issues/419) a third retry tier for bulk metadata runs, measured from production logs — request spacing was shown NOT to be the lever; [#449](https://github.com/guycorbaz/mybibli/issues/449) the startup log shortens the build commit to 7 characters. **One additive migration** (`titles.metadata_source`, nullable — pre-existing rows read as "unknown provenance"); see the v1.15.0 section below. The prior release [`v1.14.1`](https://github.com/guycorbaz/mybibli/releases/tag/v1.14.1) — a one-issue patch on v1.14.0 (shipped 2026-07-28): [#447](https://github.com/guycorbaz/mybibli/issues/447) the application now logs its version, build commit and profile at startup, so a log file read in isolation identifies the build that produced it; no migration. The underlying minor [`v1.14.0`](https://github.com/guycorbaz/mybibli/releases/tag/v1.14.0) — a cataloging-fix and bibliographic-coverage minor on v1.13.0 (shipped 2026-07-28): [#440](https://github.com/guycorbaz/mybibli/issues/440) volume labels no longer attach to the previous title when several UPC items are catalogued in a row, [#441](https://github.com/guycorbaz/mybibli/issues/441) a deleted active title now says so instead of blaming the scanned label, [#442](https://github.com/guycorbaz/mybibli/issues/442) a deleted volume's V-code label is reusable without an admin purge (refused when it carries loan history), and [#439](https://github.com/guycorbaz/mybibli/issues/439) Library of Congress MARC 21 records fill the UNIMARC-aligned fields for English-language books that the BnF does not hold — the two national libraries now complete each other field by field, first source wins; **no migration**; note that French-prefix titles the BnF could not describe stay empty, LoC does not hold them either; see the v1.14.0 section below. The prior release [`v1.13.0`](https://github.com/guycorbaz/mybibli/releases/tag/v1.13.0) — a UNIMARC-themed feature minor on v1.12.0 (shipped 2026-07-24): [#389](https://github.com/guycorbaz/mybibli/issues/389) six UNIMARC-aligned cataloging fields (200$f, 205$a, 225$a/$v, 300$a, 500$a) captured from the BnF at scan time and shown on the title page, plus a Health-tab *Backfill metadata from BnF* bulk action for already-cataloged titles; [#434](https://github.com/guycorbaz/mybibli/issues/434) per-title cataloging log summary; one additive migration; record import/export (ISO 2709 / UNIMARC XML) was dropped on 2026-08-17 — SIGB record exchange makes no sense for a household library; see the v1.13.0 section below. The prior release [`v1.12.0`](https://github.com/guycorbaz/mybibli/releases/tag/v1.12.0) — a 2-issue feature minor on v1.11.0 (shipped 2026-07-11, same day as v1.11.0): [#427](https://github.com/guycorbaz/mybibli/issues/427) BnF Couvertures + Inventaire.io cover fallbacks (live-tested to recover ~half of the previously unfindable FR/CH covers), [#428](https://github.com/guycorbaz/mybibli/issues/428) highest used V/L-code line on /catalog for label printing; no migration; see the v1.12.0 section below. The prior release [`v1.11.0`](https://github.com/guycorbaz/mybibli/releases/tag/v1.11.0) — a 4-issue minor on v1.10.0 (shipped 2026-07-11), bundling the July production-log review: [#418](https://github.com/guycorbaz/mybibli/issues/418) persistent session cookie + admin-configurable inactivity timeout, [#419](https://github.com/guycorbaz/mybibli/issues/419) bulk cover-refetch pacing/back-off/summary, [#416](https://github.com/guycorbaz/mybibli/issues/416) auto-purge unblocked, [#417](https://github.com/guycorbaz/mybibli/issues/417) log-noise fix; one additive settings migration; see the v1.11.0 section below. The prior release [`v1.10.0`](https://github.com/guycorbaz/mybibli/releases/tag/v1.10.0) — a single-feature minor on v1.9.1 (shipped 2026-07-01): [#9](https://github.com/guycorbaz/mybibli/issues/9) undo the last scan action (shelving a volume or activating a batch location) from the catalog feedback list within a 30-second window; no migration; see the v1.10.0 section below. The prior line [`v1.9.1`](https://github.com/guycorbaz/mybibli/releases/tag/v1.9.1) — a 2-issue patch on v1.9.0 (shipped 2026-06-11): [#403](https://github.com/guycorbaz/mybibli/issues/403) the last two client-side messages (modal-busy guard, server-error feedback) now come from the #i18n-bundle data island and localize in German/Italian, [#412](https://github.com/guycorbaz/mybibli/issues/412) CI de-flake of the recent-activity boundary tests; see the v1.9.1 section below. The underlying feature minor [`v1.9.0`](https://github.com/guycorbaz/mybibli/releases/tag/v1.9.0) — 4 issues on v1.8.6 (same day): [#367](https://github.com/guycorbaz/mybibli/issues/367) saved custom searches (name and save a home search composition — free text + filter + sort — then re-run, rename or delete it from a dropdown next to the sort control), [#396](https://github.com/guycorbaz/mybibli/issues/396) per-provider metadata-timeout overrides in /admin > System, [#406](https://github.com/guycorbaz/mybibli/issues/406) admin log-level form no longer 409s on consecutive saves, and [#405](https://github.com/guycorbaz/mybibli/issues/405) transport-crate log noise capped at the default level; see the v1.9.0 section below. The feature build for v1.1 through v1.8 is complete; v1.8 was the last *themed* minor and the project is in **GH-issue-driven polish mode** — subsequent minors (v1.9.0 is the first) bundle the issue-driven work that crossed the feature threshold, patches ship independently for production bugs, and new feature CRs queue in the parking lot until a coherent theme emerges or a roadmap-slotted candidate (e.g., the v2.0 classification refactor) is greenlit. Release-by-release history: the per-version sections below, the [GitHub releases page](https://github.com/guycorbaz/mybibli/releases), and chapter 8 of the user manual.

Open issues against the live line:

*(none)*

**The backlog is empty** — no open issue, for the first time since the project opened. New work starts from a fresh report.

## v1.18.0 — management labels *(shipped)*

Shipped 2026-08-18. One issue, **one additive migration**, three reviewed tranches.

- [#443](https://github.com/guycorbaz/mybibli/issues/443) — **a vocabulary for the work in progress.** Genres classify a book for the reader; volume states describe a copy's condition. Neither says "I am not sure this record is right, come back to it". Labels do: an admin defines the vocabulary in *Administration → Reference data*, and a librarian puts any of it on a title or on a volume, from the same list. A `/labels` page shows the vocabulary with its usage, drills down to the members of one label, and links through to the item so it can be fixed.

  Three things about the design are worth recording, because each was a decision rather than a default:

  **Usage is the sum of two join tables.** A label spans titles and volumes, so the deletion guard counts both. A guard looking at one of them would report a plausible zero for a label carried by fifty volumes and delete it off every shelf without a word — the failure mode that shaped the whole model layer, and the one the integration tests exercise in each direction.

  **The member list is two sections, not one merged table.** A title row wants cover / title / contributor / genre; a volume row wants V-code / parent title / location / state. Merging them would either duplicate columns or flatten to their intersection. Each section pages independently, and an empty one states its emptiness rather than disappearing — "no volumes carry this label" has to stay distinguishable from "that section failed to load".

  **Anonymous visitors are excluded by not asking.** The detail handlers skip the label queries entirely rather than loading them and hiding them, so there is no code path on which a label name reaches a visitor. The nav entry sits inside the Librarian+ block, and the endpoints refuse rather than merely hiding the affordance.

  Two pre-existing guards caught defects in this work that review had not: the #424 contrast audit rejected `text-stone-400` in the new fragment and named the correct tier, and `responsive-layouts.spec.ts` measured an 86px tablet overflow from the new nav entry. **The nav bar therefore switches to its menu at 1024px instead of 768px** — a deliberate change affecting every role, chosen over hiding the single entry, which would have left `/labels` reachable by URL only between 768 and 1023px.

## v1.17.0 — metadata clarity *(shipped)*

Shipped 2026-08-18. Two issues, **no migration**.

Both came from the same production user, reported eight months apart, and auditing each one before writing code changed what got written. Neither turned out to be the defect it was filed as; both were the interface failing to say something true.

- [#202](https://github.com/guycorbaz/mybibli/issues/202) — **a failed lookup now says why.** The chain has distinguished every failure cause since v1.11.0 — no result, throttled, unavailable, network failure, per-provider timeout — but each one went to the log and reached the user as a single "no metadata found". That one message covered three situations calling for opposite reactions: nobody holds the book (catalogue it by hand; no key and no retry will help), a source was busy or too slow (worth pressing again in a few minutes), or a source was never asked for want of an API key (one field in Administration → System). Pressing *Re-download metadata* now names which, and names the sources — untranslated, as in the v1.15.0 provenance badge. No new detection logic was written: the diagnosis existed, only its retention was missing.

  One conflation had to be repaired at the source. OMDb and TMDb, given no API key, returned the same "found nothing" as a genuine empty search — the very distinction this work exists to draw. They now report the missing key as such.

  The diagnosis is produced on demand rather than stored: provider availability is episodic, so a reason recorded three weeks ago would describe a world that no longer exists — and persisting it would have cost a migration for a worse answer. **Tier 3 (retrying one specific provider) was dropped**: once the message names the source and the reason, a targeted retry does what the ordinary one does, with one more control to learn.

- [#206](https://github.com/guycorbaz/mybibli/issues/206) — **which of genre and Dewey a fetch may overwrite.** The report asked that the genre stop being overwritten automatically. It already was not, three times over: no provider returns a genre, `genre_id` is absent from the fetch UPDATE, and the edit-tracking helper excludes it. There was no defect. What the form never said is that the neighbouring field behaves the *opposite* way — the Dewey code **is** provider-filled, and keeps being so until the first manual edit records the value as yours. Read from the form alone, that asymmetry inverts, which is very likely how the report came to be written.

  Both fields now sit in a **Classification** section stating the difference; publication date, being bibliographic, stays outside it. A guard in the code fails the build if a metadata fetch ever writes the genre — the interface makes a promise, and nothing else was enforcing it.

## v1.16.0 — K10plus bibliographic coverage + admin recovery *(shipped)*

Shipped 2026-08-17. Four issues, **no migration**, two release candidates.

- [#450](https://github.com/guycorbaz/mybibli/issues/450) — after v1.14.0 added the Library of Congress, 38 English-prefix titles still carried none of the six UNIMARC-aligned fields. Six catalogues were probed before one qualified: Jisc Library Hub is licence-gated (403 anonymously), the British Library's SRU and linked-data endpoints no longer answer at all, Harvard LibraryCloud holds none of them, Open Library was already ruled out. **K10plus**, the German union catalogue, answers anonymous SRU with MARC 21 XML under CC0 — the cleanest licence in the chain — and reads through the existing `metadata::marc` parser, so it needed only a provider declaring `supplies_marc_zones()`. It recovered 18 of the 38, every one of them, and 45 titles / 100 zones across the whole catalog. The requests are gated by ISBN prefix (978-0/1/2/3) so the 1 req/s limiter only paces calls that can plausibly hit; the 978-2 gate acts as a free second opinion on titles the BnF answered without zones, and earned its place with 9 completions.
- [#459](https://github.com/guycorbaz/mybibli/issues/459) — an administrator locked out of a single-tenant install previously needed a MariaDB console to get back in. `MYBIBLI_RESET_ADMIN` reads once at startup, resets exactly one account, and is consumed — leaving the variable set in `docker-compose.yml` does not re-arm it.
- [#457](https://github.com/guycorbaz/mybibli/issues/457) — the next-free L-code proposal ignored soft-deleted rows, so it could propose a code a deleted location still held and the save then failed on the UNIQUE constraint. It now counts deleted rows and proposes the lowest genuinely free code.
- [#458](https://github.com/guycorbaz/mybibli/issues/458) — four E2E specs failed intermittently, which is worse than failing outright: it trains the reader to re-run rather than to read. Three distinct root causes, all fixed at the source rather than papered over with waits.

**On the two release candidates.** rc.1 was withdrawn before publication: its own release gate caught a rate-limiter contention bug — the trait-exposed 1 req/s limiter serialised the zone-completion pass under parallel load, 82 K10plus calls per E2E run. rc.2 added the prefix gate and shipped to production for measurement. Its backfill log then settled the decision, and did so without the database: the `Title metadata resolved` event carries `unimarc_zones_populated`, so the affected population reconstructs from the log files alone — the 29 July baseline of 38 was reproduced 19 days after the fact, validating the instrument before it was trusted on the new run. The pre-registered acceptance threshold (19 of 38) was missed by a single title; it was overridden deliberately, because it scored the provider solely on the hardest population in the catalog and ignored the 27 further titles it completed elsewhere. The 20 titles that remain empty cluster by publisher — two UK small presses and a scatter of print-on-demand imprints — and are recorded on the issue as not automatically recoverable by any catalogue source probed.
