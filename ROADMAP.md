...
**Current stable: [`v1.7.8`](https://github.com/guycorbaz/mybibli/releases/tag/v1.7.8)** — the eighth patch on top of v1.7.0 ("Reach more users, debug more easily" — German + Italian UI translations and persistent log directory with rotation, shipped 2026-05-21). v1.7.8 ships a hardening bundle with one user-visible UX fix — concurrent-delete modals on `/borrower/:id`, `/contributor/:id`, and the return-loan modal now surface inline feedback instead of silently no-op'ing when the row is gone (#136) — plus three code-review-finding hardenings (#39 atomic login session swap, #47 four CSRF coverage gaps, #217 modal Confirm scope tightening). The originally-planned feature build for v1.1 through v1.6 is complete and v1.7 is currently the last themed minor on the roadmap; the project is in **GH-issue-driven polish mode** — no next themed minor is currently scoped. Patches ship independently for production bugs; new feature CRs are queued in the parking lot until a coherent theme emerges or a roadmap-slotted candidate (e.g., the v2.0 classification refactor) is greenlit.
