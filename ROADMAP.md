...
**Current stable: [`v1.7.6`](https://github.com/guycorbaz/mybibli/releases/tag/v1.7.6)** — the sixth patch on top of v1.7.0 ("Reach more users, debug more easily" — German + Italian UI translations and persistent log directory with rotation, shipped 2026-05-21). v1.7.6 bundles two user-visible feature fixes — editable `media_type` on `/title/:id` (#331, a BD scanned via ISBN can finally be reclassified) and manual cover upload (#335, Librarian+ drops a JPG/PNG/WebP/GIF as the safety net for titles where no provider has a cover — Open Library Covers fallback misses most French BDs / mangas / small-publisher technical books) — plus three hardening commits (#109, #40, #36). The originally-planned feature build for v1.1 through v1.6 is complete and v1.7 is currently the last themed minor on the roadmap; the project is in **GH-issue-driven polish mode** — no next themed minor is currently scoped. Patches ship independently for production bugs; new feature CRs are queued in the parking lot until a coherent theme emerges or a roadmap-slotted candidate (e.g., the v2.0 classification refactor) is greenlit.
...
- [#333](https://github.com/guycorbaz/mybibli/issues/333) — Cover-refetch bulk action insuffisante : 73/146 prod titles without a cover (Open Library Covers FR-catalog index gap; v1.7.6 #335 ships the manual-upload safety net but the structural fix — likely BDGest cover-only fetch — needs its own slot).
- [#334](https://github.com/guycorbaz/mybibli/issues/334) — Expose metadata-chain + provider-health timeouts in `/admin > System` (env-var-only today; pattern v1.7.1 #308 log-level UI).
- [#336](https://github.com/guycorbaz/mybibli/issues/336) — Display cover thumbnail + volume number in the series detail view (visual gap detection for series collectors).
